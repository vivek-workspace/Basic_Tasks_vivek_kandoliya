...
//======================= Global Objects to Store

const data = {}
router.post('/savebasic', varifyUser,async (req,res) => {
    const basicData = req.body;
    const id = await insertBasicDetails(basicData);
    console.log(id)
    res.json(id);
})

router.post('/saveedu', varifyUser,async (req,res) => {
...
router.get('/getusr', varifyUser, async (req, res) => {
    let promisedQuery = util.promisify(con.query).bind(con);
